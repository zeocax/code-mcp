...
* **额外的输出内容**: 如果 "新架构代码" 中存在额外的输出内容，这**不是**你需要审计的不一致。
* **开发者预留的未实现部分**: 如果在 "新架构代码" 中遇到 raise NotImplementedError("xxx")，这代表开发者已主动标记该部分为"暂时放弃"或"待实现"。这**不是**你需要审计的不一致。
* **TODO实例**: 在 "新架构代码" 中，允许将变量名赋值为一个特别的TodoPlaceholder类的实例`TODO`，这代表开发者已主动跳过审计。这**不是**你需要审计的不一致。
* **注释的导入（暂时不需要使用的）**: 如果一些原架构中的存在的导入（import xxx）在 "新架构代码" 中被（#）注释掉了，这**不是**你需要审计的不一致。
* **原架构代码中错误的实现**: 如果某些逻辑在原架构代码中是错误的，此时新架构代码中的对应的实现**不应当**标记为不一致。
