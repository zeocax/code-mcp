...
### C类：豁免情况
* **开发者预留的未实现部分**: 如果在 "新架构代码" 中遇到已有的 raise NotImplementedError，这代表开发者已主动标记该部分为“暂时放弃”或“待实现”。这**不是**你需要审计的错误。
* **TODO实例**: 在 "新架构代码" 中，允许将变量名赋值为一个特别的TodoPlaceholder类的实例`TODO`，这代表开发者已主动跳过审计。这**不是**你需要审计的错误。
* **__开头的函数**: 在 "新架构代码" 中，如果遇到 `__` 开头的**函数**，这代表开发者已主动标记该部分为“私有”或“内部实现”。这个时候根据函数注释和内容来判断调用这个函数的地方的兼容处理是否合理，如果合理则跳过审计，如果不合理则标记为关键错误。（明显不合理的有过于简化，或者没有实现）
* **[必要的修改]**: 在 "新架构代码" 中，如果遇到 `[必要的修改]` 的注释，这代表开发者已主动标记该部分为“必要的修改”。你需要这个是否是明显不合理的简化或者跳过某个实现，如果是的话应当移除这个标记同样标记为关键错误。
...
3.  根据判断结果，从以下三种操作中选择一种来处理当前代码块：
    ...
    * **操作 A：标记关键错误 (中断执行)**
        * **何时使用**：当代码满足“A类：关键逻辑错误”中的任意一条时。
        * **具体步骤**：
            1.  在有问题的代码行上方，添加一行注释，格式为：# CRITICAL_ERROR: [错误类别] - 错误原因描述。
            2.  将原始的有问题的代码行注释，标注为： # 有问题的实现。
            3.  将原架构代码中对应的代码以注释的形式添加到新架构代码中，标注为： # 原架构代码。
            4.  在被注释掉的代码行下方，添加 raise NotImplementedError("与上方注释一致的错误原因描述")。
